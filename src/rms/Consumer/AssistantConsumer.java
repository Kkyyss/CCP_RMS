...
public class AssistantConsumer implements Runnable {
    ...
    public AssistantConsumer(
            String name, Assistant assistant, BlockingQueue<Customer> tableCustomer) {
        this.name = name;
    ...
    public void run() {
        while (true) {
            if (closingTime && tableCustomer.isEmpty()) {
                serve();
            ...
            if (!assistant.isIdle()) {
                ...
                for (int i = 0; i < conf.getNumberOfTables(); i++) {
                    ...
                    for (int j = 0; j < t.getGlasses(); j++) {
                        glasses++;
                        if (t.getUnit() < conf.getTableCapacity()) {
                            t.addUnit(1);
                        }
                        t.decrementGlasses();
